...
/**
 * 【气球游戏】小Q在进行射击气球的游戏，如果小Q在连续T枪中打爆了所有颜色的气球，将得到一只QQ公仔作为奖励。 （每种颜色的气球至少被打爆一只）。这个游戏中有m种不同颜色的气球，编号1到m。小Q一共有n发子弹，然后连续开了n枪。
 ...
 * <p>
 * 示例  输入： 12 5 2 5 3 1 3 2 4 1 0 5 4 3 输出：  6
 * <p>
 * 合格题目有个限制，输入的颜色和参数 5 是一样的，如果颜色更多，比如颜色一共有10种，我们需要 n 个颜色最少的连续数字，事情变的有意思了。 如果一共有m个颜色，要求n个不同颜色的最小数字。m
 * >= n 。事实上，这种情况包含了前一种情况。
 *
 * @author why
 */
public class QiQiu {
  ...
  public static int shootCount2(int colors, int[] shootColors) {
  ...
  public static int shootCount(int colors, int[] shoots) {
      ...
      if (shoots == null || shoots.length == 0) {
          return -1;
      }
      if (shoots.length < colors) {
          return -1;
      }
      if (colors == 1) {
          return 1;
